...
/**
 * 类名:  ChatRoomDetailsActivity
 * <br/> 功能描述: 聊天室
 * <br/> 作者: MouTao
 * <br/> 时间: 2017/6/14
 */
public class ChatRoomDetailsActivity extends EMBaseActivity implements OnClickListener {
            ...
            switch (requestCode) {
                ...
                case REQUEST_CODE_EDIT_CHAT_ROOM_NAME:
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            try {
                                EMClient.getInstance().chatroomManager().changeChatRoomSubject(roomId, data
                                        .getStringExtra("data"));
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                ...
                case REQUEST_CODE_EDIT_CHAT_ROOM_DESCRIPTION:
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            try {
                                EMClient.getInstance().chatroomManager().changeChatroomDescription(roomId, data
                                        .getStringExtra("data"));
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
            ...
            public void run() {
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            progressDialog.dismiss();
                            Toast.makeText(getApplicationContext(), "Failed to quit group: " + e.getMessage(), Toast
                                    .LENGTH_LONG).show();
                        }
                    });
                }
                ...
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            ...
                            Button destroyButton = (Button) ChatRoomDetailsActivity.this.findViewById(R.id
                                    .btn_destroy_chatroom);
                            destroyButton.setVisibility(EMClient.getInstance().getCurrentUser().equals(room.getOwner
                                    ()) ? View.VISIBLE : View.GONE);

                            RelativeLayout changeChatRoomNameLayout = (RelativeLayout) findViewById(R.id
                                    .rl_change_chatroom_name);
                            RelativeLayout changeChatRoomDescriptionLayout = (RelativeLayout) findViewById(R.id
                                    .rl_change_chatroom_detail);
                            changeChatRoomNameLayout.setVisibility(isCurrentOwner(room) ? View.VISIBLE : View.GONE);
                            changeChatRoomDescriptionLayout.setVisibility(isCurrentAdmin(room) ? View.VISIBLE : View
                                    .GONE);
                        }
                    });
                }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.rl_change_chatroom_name:
                //				startActivityForResult(new Intent(this, EditActivity.class).putExtra("data", room
                // .getName()).putExtra("title", "edit chat room name"),
                //                REQUEST_CODE_EDIT_CHAT_ROOM_NAME);
                break;
            case R.id.rl_change_chatroom_detail:
                //				startActivityForResult(new Intent(this, EditActivity.class).putExtra("data", room
                // .getDescription()).putExtra("title", "edit chat room detail"),
    ...
    private final int[] ids = {R.id.menu_item_transfer_owner, R.id.menu_item_add_admin, R.id.menu_item_rm_admin, R.id
            .menu_item_remove_member, R.id.menu_item_add_to_blacklist, R.id.menu_item_remove_from_blacklist, R.id
            .menu_item_mute, R.id.menu_item_unmute};
    ...
    private class OwnerAdminAdapter extends ArrayAdapter<String> {
        ...
        public View getView(final int position, View convertView, final ViewGroup parent) {
            ...
            id_background.setBackgroundColor(convertView.getResources().getColor(position == 0 ? R.color
                    .holo_red_light : R.color.holo_orange_light));
            button.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    boolean[] adminVisibilities = {true,       //R.id.menu_item_transfer_owner,
    ...
    Dialog createMemberMenuDialog() {
        ...
        int ids[] = {R.id.menu_item_add_admin, R.id.menu_item_rm_admin, R.id.menu_item_remove_member, R.id
                .menu_item_add_to_blacklist, R.id.menu_item_remove_from_blacklist, R.id.menu_item_transfer_owner, R
                .id.menu_item_mute, R.id.menu_item_unmute};
                            ...
                            try {
                                switch (v.getId()) {
                                    case R.id.menu_item_add_admin:
                                        EMClient.getInstance().chatroomManager().addChatRoomAdmin(roomId, 
                                                operationUserId);
                                        break;
                                    case R.id.menu_item_rm_admin:
                                        EMClient.getInstance().chatroomManager().removeChatRoomAdmin(roomId, 
                                                operationUserId);
                                        break;
                                    ...
                                    case R.id.menu_item_mute:
                                        ...
                                        EMClient.getInstance().chatroomManager().muteChatRoomMembers(roomId, 
                                                muteMembers, 20 * 60 * 1000);
                                        break;
                                ...
                                runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        Toast.makeText(ChatRoomDetailsActivity.this, e.getDescription(), Toast
                                                .LENGTH_SHORT).show();
                                    }
                                });
                                e.printStackTrace();
    ...
    private class MemberAdapter extends ArrayAdapter<String> {
        ...
        public View getView(final int position, View convertView, final ViewGroup parent) {
            ...
            if (isInMuteList(username)) {
                ...
                id_background.setBackgroundColor(convertView.getResources().getColor(R.color.black));
            } else {
            ...
            button.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    boolean[] normalVisibilities = {false,      //R.id.menu_item_transfer_owner,
                    ...
                    boolean[] blackListVisibilities = {false,      //R.id.menu_item_transfer_owner,
                            false,      //R.id.menu_item_add_admin,
                            false,      //R.id.menu_item_rm_admin,
                            false,      //R.id.menu_item_remove_member,
                            false,      //R.id.menu_item_add_to_blacklist,
                    ...
                    boolean[] muteListVisibilities = {false,      //R.id.menu_item_transfer_owner,
                            isCurrentOwner(room) ? true : false,       //R.id.menu_item_add_admin,
                            false,      //R.id.menu_item_rm_admin,
                            true,       //R.id.menu_item_remove_member,
                            true,       //R.id.menu_item_add_to_blacklist,
    ...
    public void onDestroyChatRoomClick(View v) {
        //TODO 删除聊天室
        //        startActivityForResult(new Intent(this, ExitGroupDialog.class).putExtra("deleteToast", getString(R
        // .string.dissolution_group_hint)),
            ...
            if (roomId.equals(ChatRoomDetailsActivity.this.roomId)) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Toast.makeText(ChatRoomDetailsActivity.this, "onMuteListAdded: " + sb.toString(), Toast
                                .LENGTH_SHORT).show();
                    }
                });
                updateRoom();
        ...
        public void onMuteListRemoved(final String chatRoomId, final List<String> mutes) {
            if (roomId.equals(ChatRoomDetailsActivity.this.roomId)) {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(ChatRoomDetailsActivity.this, "onMuteListRemoved: " + sb.toString(), Toast
                                .LENGTH_SHORT).show();
                    }
                });
                refreshMembersAdapter();
        ...
        public void onAdminAdded(final String chatRoomId, final String admin) {
            if (roomId.equals(ChatRoomDetailsActivity.this.roomId)) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(ChatRoomDetailsActivity.this, "onAdminAdded: " + admin, Toast.LENGTH_SHORT)
                                .show();
                    }
                });
                updateRoom();
        ...
        public void onAdminRemoved(final String chatRoomId, final String admin) {
            if (roomId.equals(ChatRoomDetailsActivity.this.roomId)) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(ChatRoomDetailsActivity.this, "onAdminRemoved: " + admin, Toast.LENGTH_SHORT)
                                .show();
                    }
                });
                updateRoom();
        ...
        public void onOwnerChanged(final String chatRoomId, final String newOwner, final String oldOwner) {
            if (roomId.equals(ChatRoomDetailsActivity.this.roomId)) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(ChatRoomDetailsActivity.this, "onOwnerChanged newOwner:" + newOwner + "  " + 
                                "oldOwner" + oldOwner, Toast.LENGTH_SHORT).show();
                    }
                });
                updateRoom();
            }
