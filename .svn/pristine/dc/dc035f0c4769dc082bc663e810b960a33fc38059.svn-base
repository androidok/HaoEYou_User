...
public class AddRecordActivity extends BaseActivity implements ItemListener<LocalMedia>, AddView {
    ...
    protected void initEventAndData() {
        from = getIntent().getExtras().getString(Common.FROM, "");
        patient_id = getIntent().getExtras().getString("patient_id", "");
        medical_history_id = getIntent().getExtras().getString("medical_history_id", "");
    ...
    private void checkAddStatus() {
        if (TextUtils.isEmpty(mMessage.getText()) || selectMedia.size() ==0) {
            mMore.setTextColor(Color.parseColor("#F1F1F1"));
            mMore.setClickable(false);
        } else {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.title_back:
                ...
                break;
            case R.id.more:
                if (HistoryDetailActivity.TAG.equals(from)) {
                    if (TextUtils.isEmpty(medical_history_id))
                        return;
                    ((AddPresenter) mPresenter).addFile(medical_history_id, new AddMedicalBean(patient_id, mMessage
                            .getText().toString(), Common.TOKEN), selectMedia);
                } else {
    ...
    @Override
    public void uploadFileFinish(ArrayList<ImageUrlResponseBean> list) {
        if (HistoryDetailActivity.TAG.equals(from)) {
            Intent itt = new Intent();
            Bundle bundle = new Bundle();
            bundle.putSerializable("IMAGE_LIST", list);
     ...
     * @param from 从哪儿跳来的
     */
    public static void startAction(Context ct, String medical_history_id, String patient_id, String oldMessage, 
                                   String from) {
        Intent itt = new Intent(ct, AddRecordActivity.class);
        Bundle bundle = new Bundle();
        bundle.putString(Common.FROM, from);
        bundle.putString("patient_id", patient_id);
        bundle.putString("medical_history_id", medical_history_id);
