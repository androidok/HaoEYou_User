...
package com.haoeyou.user.widget.smoothcheck;
...
public class SmoothCheckBox extends View implements Checkable {
    ...
    private void init(AttributeSet attrs) {
        ...
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.SmoothCheckBox_stroke_width, CompatUtils.dp2px(getContext
                (), 0));
        ta.recycle();
...
//        });
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mLeftLineDistance = (float) Math.sqrt(Math.pow(mTickPoints[1].x - mTickPoints[0].x, 2) + Math.pow
                (mTickPoints[1].y - mTickPoints[0].y, 2));
        mRightLineDistance = (float) Math.sqrt(Math.pow(mTickPoints[2].x - mTickPoints[1].x, 2) + Math.pow
